...
//#define VERSION "mdh V4.00 Sept 12 2022"      // Tetiana
#define VERSION "mdh V4.07 June 02 2023"         // Tetiana
...
/*
  ...
  4.00     End positions X,Y,Z of each ion, the final energy E and total range along the path are saved into a text file to analyze

  4.01     Trajectories of BackScattered ions are saved into a text file for each backscattered ion. The name of the file contains the number (Nrec) if the ion.
  4.02     Information about only backscattered ions is stored into a text file.
           Stored parameters are  the number (Nrec) of the ion, the end position X,Y,Z, the original (initial) direction: theta ans phii,
           the final energy, the total range along the path, velocity vector components vx, vy, vz,


  4.03     Additional information about BS ions is stored. Stored parameters are
           the number (Nrec) of the ion, the end position X,Y,Z, the original (initial) direction: theta ans phii,
           the final energy, the total range along the path, velocity vector components vx, vy, vz,
           the deposited energy, the total nuclear deposited anergy and the total electronic deposited energy
   ...
  4.05     the  "-fullTrajectories"    flag was added.
  4.06     tmp version

  4.07     the format of output data files was slightly changed ( according to data processing algorithms). No important changes of the functionality.
...
/*large atom tables for the simulation cells*/
   ...
   printf(" initparams(type,&unit,&time,&gen,&file); line 169 in mdh  is OK!\n");
    ...
    if (file.printBS )
       {
            ...
            {
                recresult.bsArray[ii].x = INT_MIN;
                recresult.bsArray[ii].y = INT_MIN;
                recresult.bsArray[ii].z = INT_MIN;

                recresult.bsFD[ii]  = 0.0;
                recresult.bsFDe[ii] = 0.0;
                recresult.bsFDn[ii] = 0.0;
                recresult.bsFDf[ii] = 0.0;
            }
            recresult.kk = 0;
       }
   ...
   for (nrec=1;nrec<=reccalc.Ncalc;)
   {
        ...
        }  //  end of checking  (check if first layer has ...)
...
//..................................................................
	...
	//0709 2022
	if ( recoil.range > reccalc.Zmax || recoil.range > layers.layer[layers.Nlayers-1].maxz)
	{
		//  Tetiana //0709 2022
		//  tmp        (commented  ~ in the beginning of October 2022 because  ...   )
			//FILE   *myMaxZ = fopen("my_tmp_MaxZ.txt", "at");
			//fprintf(myMaxZ, "%.4f %.4f %.4f  %.4f \n", recoil.sx, recoil.sy, recoil.sz, recoil.energy);
			//fclose (myMaxZ);
		//
	 ...
	 if (recoil.sz<(2.0*reccalc.Startmin.z))
	 {
	 	issput=True;
	    break;
	 }
	...
	// +++++++++++++ Tetiana
	 if ( recoil.sz <= (reccalc.Startmin.z) )
	 {
        FILE   *myRealMinZ = fopen("BackScatteredIons.txt", "at");
        // n_X_Y_Z_E_R_vx_vy_vz_TotDep_NuDep_ElDep
        int mm=0;
        double sFD_each_BS_ion  = 0.0;
        double sFDn_each_BS_ion = 0.0;
        double sFDe_each_BS_ion = 0.0;
        double sFDf_each_BS_ion = 0.0;

        for (mm=0; mm < recresult.i_steps; mm++)
        {
          sFD_each_BS_ion  += recresult.bsFD[mm];
          sFDn_each_BS_ion += recresult.bsFDn[mm];
          sFDe_each_BS_ion += recresult.bsFDe[mm];
          sFDf_each_BS_ion += recresult.bsFDf[mm];
        }

        fprintf(myRealMinZ, "%d %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f\n",
                recoil.n, recoil.sx, recoil.sy, recoil.sz, recoil.energy, recoil.r_path,
                recoil.vx, recoil.vy, recoil.vz, sFD_each_BS_ion, sFDn_each_BS_ion, sFDe_each_BS_ion ); // 02 Nov 22  "Nuclear and electronic"
		fclose (myRealMinZ);
	 ...
	 if (reccalc.type!=2) {
	   ...
	   if (reccalc.Trange == 1) {
	     if (recoil.range<(2.0*recoil.z0/cos(recoil.theta) ))
	     {
            issput=True; break;
	     }
	   }
         ...
         if (file.printcoords) printcoint(at,atflag,nat,&box,t,reccalc.Outputstep);
          ...
          //   Tetiana:  we have "normal" printrec, at normal conditions without bugs but using dm debug-flag  or printrec-flag
         if (file.printrec || maxdebug)  printrec(&recoil,t,reccalc.Outputstep);

         //+++++   Tetiana:  we have "normal" printrec but for ALL steps of BS ions
         if ( file.printBS )  printBS(&recoil, &file, &recresult,&elstop, &reccalc, t, reccalc.Outputstep);    // Tetiana
