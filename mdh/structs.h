...
struct gen {
   ...
   logical useTriton; //Tetiana:  // bool variable: Do we use Triton in this task?
};
...
struct recresult {
  ...
  double surfFDn;   /*deposited energy in the surface layer for sputtering*/
   ...
   //+++++Tetiana
   point bsArray[MAX_BSTRACK_SIZE];     // Tetiana:  I have to store all trajectories for backscattered ions
   unsigned long int kk;                // Tetiana:  I need a current number of tiny step for each BS ion
   double bsFDn[MAX_BSTRACK_SIZE], bsFDe[MAX_BSTRACK_SIZE], bsFD[MAX_BSTRACK_SIZE], bsFDf[MAX_BSTRACK_SIZE]; // Tetiana:  arraysh collect FD, FDn, FDe, FDf for each tiny step
   unsigned long int i_steps;    // Tetiana: Counter of steps, an index of arrays elements
   //+++++
};
...
/* Output files. fpo=stdout and fpdb=stderr assumed */
   ...
   char outf[120],startf[120],rangef[120],depenf[120],stopf[120],transf[120]; //Glen
   ...
   FILE *fpo,*fps,*fpra,*fpde,*fpst,*fpav,*fpdb,*fprs,*fprst,*fprho,*fpcollisions, *ftra; //Glen
   ...
   logical recoilspec,printrecend,printdens, printcustomTrans; //Glen

   //+++++ Tetiana
   logical printBS;     // Tetiana:  logical variable (true or false)  - Do we need information about backscattered ions ?
   logical printBS_fullTrajectories;     // Tetiana:  logical variable (true or false)  - Do I need a whole trajectory for each backscattered ion ?
   //+++++
