...
//#include <vector>     //Tetiana: I cannot use <vector> (or <list>) from STL
//using namespace std
...
//+++++ Tetiana, Oct 2022 :
printBS(recoil, file, recresult, elstop, reccalc, t, n)
struct recoil *recoil;
struct file *file;
struct recresult *recresult;
struct elstop *elstop;
struct recclc *reccalc;
real t;
int n;
{
   static int c=0;
   ...
   if (n>0) {
      ...
      {
         printf("Debug information: return from printBS because c<n   (c is static int, counter) \n"); // Tetiana
         return;
      }
   }
   else c=0;

   recresult->bsArray[recresult->kk].x = recoil->sx;
   recresult->bsArray[recresult->kk].y = recoil->sy;
   recresult->bsArray[recresult->kk].z = recoil->sz;

   recresult->kk = recresult->kk +1;
