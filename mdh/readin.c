...
struct gen *gen;
{
  ...
  //+++++ Tetiana
  if (gen->useTriton == False)
        RIC(gen->seed,  rand(), " rand(), Tetiana : Seed number for random number generator is received using time(NULL) \n\n");
  else
        printf("we use the seed from a command line in our bash script on Triton,  useTriton flag is %d, seed=%d, \n", gen->useTriton, gen->seed);
