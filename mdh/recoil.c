...
struct type *type;
{
   ...
   if (file->printBS )
   {
     ...
     {
       recresult->bsArray[ii].x = INT_MIN +1;
       recresult->bsArray[ii].y = INT_MIN +1;
       recresult->bsArray[ii].z = INT_MIN +1;
...
real t;
{
   ...
   //+++++ Tetiana
   recresult->bsFD[recresult->i_steps]  = FD;
   recresult->bsFDn[recresult->i_steps] = FDn;
   recresult->bsFDe[recresult->i_steps] = FDe;
   recresult->bsFDf[recresult->i_steps] = FDf;
   recresult->i_steps +=1;
...
*/
   ...
   if (file->printrecend) {
    ...
    {
      FILE *myFilePos_E_R;
      myFilePos_E_R = fopen("ALL_sx_sy_sz_E_r.txt", "at");      // 12 Oct 2022 :  for all stopped ions, not only BS
      fprintf(myFilePos_E_R, "%.2f %.2f %.2f %.2f %.2f\n",
      recoil->sx, recoil->sy, recoil->sz,
      recoil->energy,
      recoil->r_path      // total path (for monocrystalline only !!! )
    );

    fclose (myFilePos_E_R);
    ...
    if (file->printBS_fullTrajectories)   // save into *.txt file a whole trajectory of each backscattered ion    (01Feb2023)
    {
      ...
      {
	      char filename[40];
	      char ionCurrNumber[10];
          sprintf(ionCurrNumber, "%d", recoil->n);

          strcpy(filename, "BS_ion_");
	      strcat(filename,ionCurrNumber);
          strcat(filename, ".txt");

          FILE *myBS_ionTrack = fopen(filename, "at");
          int j;

          float MaxZValue = -10.0;
          FILE *myMaxZ = fopen("MaxZ_for_all_BS.txt", "at");

          for (j=0; j<MAX_BSTRACK_SIZE; j++)
          {
            if (   (  (recresult->bsArray[j].z)  < ( INT_MIN/10 +1)  )
                && (  (recresult->bsArray[j].x)  < ( INT_MIN/10 +1)  )
                && (  (recresult->bsArray[j].y)  < ( INT_MIN/10 +1)  )
               )
             {
              break;
             }
    ...
    for (ii=0; ii< MAX_BSTRACK_SIZE; ii++)
    {
      recresult->bsArray[ii].x = INT_MIN +1;  recresult->bsArray[ii].y = INT_MIN +1; recresult->bsArray[ii].z = INT_MIN +1;
      recresult->bsFD[ii]  = 0.0;  recresult->bsFDe[ii] = 0.0; recresult->bsFDn[ii] = 0.0; recresult->bsFDf[ii] = 0.0;
    }
    recresult->kk = 0;
   ...
   // Glen
   /* This file KEEPS being appended to. It MUST be moved / deleted between runs.
   The format of the output transmitted.out is given by: x y z E r vx vy vz
   It also writes the output to the console                                              */

   if (file->printcustomTrans && stopway==7) {
     fprintf(file->fpo,"Transmitted ions at %.2f %.2f %.2f E %.8g r %g v %g %g %g \n",
	     recoil->sx,recoil->sy,recoil->sz,recoil->energy,recoil->range,
	     recoil->vx,recoil->vy,recoil->vz);
     file->ftra=fopen(file->transf,"a");
     fprintf(file->ftra,"%.2f %.2f %.2f %.8g %g %g %g %g \n",
	     recoil->sx,recoil->sy,recoil->sz,recoil->energy,recoil->range,
	     recoil->vx,recoil->vy,recoil->vz);
     fclose(file->ftra);
